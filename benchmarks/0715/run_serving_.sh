...
output_csv="benchmark_results_gsm8k_0720.csv"
...
run_benchmark() {
    ...
    echo "python "$python_script" --dataset "$dataset" --request-rate "$request_rate" --draft-size "$draft_size" $chunk_prefill_flag $colocate_flag $target_attention_flag --budget $budget --temperature $temperature --drop-threshold $drop_threshold --enforce-eager" > last_run.sh
