...
def main(args: argparse.Namespace):
    ...
    if args.backend == "vllm":
        elapsed_time = run_vllm(requests, args.engine, args.target_model, args.draft_model,
        ...
        elapsed_time = run_mii(requests, args.target_model, args.tensor_parallel_size,
                               args.output_len)
    else:
...
if __name__ == "__main__":
    ...
    if args.backend == "vllm":
        ...
        if args.hf_max_batch_size is not None:
            raise ValueError("HF max batch size is only for HF backend.")
        if args.tokenizer != args.target_model:
