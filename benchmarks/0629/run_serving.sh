...
run_benchmark() {
    ...
    local output=$(python "$python_script" --dataset "$dataset" --request-rate "$request_rate" --draft-size "$draft_size" $chunk_prefill_flag $colocate_flag $target_attention_flag $output_len_flag --temperature "$temperature")
    extract_values "$output"
    echo "$dataset,$temperature,$output_len,$request_rate,$draft_size,$chunk_prefill,$colocate,$target_attention,$p50_ttft,$p99_ttft,$p50_tpot,$p99_tpot,$p50_tpt,$p99_tpt,$throughput,$latency,$preempt_flag" >> "$output_csv"
    ./slack "dataset: $dataset, temperature: $temperature, output_len: $output_len, request_rate: $request_rate, draft_size: $draft_size, chunk_prefill: $chunk_prefill, colocate: $colocate, attention: $target_attention, throughput: $throughput, latency: $latency, preempted: $preempt_flag"
}

# Initialize
initialize_csv

total_runs=$(( ${#datasets[@]} * ${#request_rates[@]} * ${#draft_sizes[@]} * ${#chunk_prefills[@]} * ${#colocates[@]} * ${#target_attentions[@]} * ${#output_lens[@]} * ${#temperatures[@]}))
current_run=0
                ...
                for chunk_prefill in "${chunk_prefills[@]}"; do
                    for request_rate in "${request_rates[@]}"; do
                        ...
                        if [ "$draft_size" -eq 0 ]; then
                            ...
                            run_benchmark "$dataset" "$output_len" "$request_rate" "$draft_size" "$chunk_prefill_flag" "" "" "$output_len_flag" "$temperature"
                            ((current_run++))
                            ...
                            for colocate in "${colocates[@]}"; do
                                for target_attention in "${target_attentions[@]}"; do
                                    ...
                                    run_benchmark "$dataset" "$output_len" "$request_rate" "$draft_size" "$chunk_prefill_flag" "$colocate_flag" "$target_attention_flag" "$output_len_flag" "$temperature"
                                    ((current_run++))
                                    ./slack "[${current_run}/${total_runs}]"
                                done
                            done
