...
for i, (target_model, draft_model, dataset) in enumerate(models_datasets):
    for j, temperature in enumerate(temperatures):
        ...
        index = f"_{i}_{j}"
        print(name+index)
        beta_dfs[name+index] = pd.read_csv(f"{name}/beta_list.csv", names=range(10000))


# Create a figure for plotting
fig, axs = plt.subplots(nrows=len(models_datasets), ncols=len(temperatures), figsize=(40, 120))
n = 4 
# Loop through all combinations
