...
def main(args: argparse.Namespace):
    ...
    llm = SpSLLM(
        ...
        lookup_file_name = f"heatmap/{args.dataset}_{args.temperature}_lookup_table"
    )
    ...
    batch_size = 64
    total_run = min(len(requests) // batch_size, 1000)
        
    for i in range(1, total_run):
        # sampled_requests = random.sample(requests, args.batch_size)
        sampled_requests = requests[batch_size * i:batch_size * (i + 1)]
