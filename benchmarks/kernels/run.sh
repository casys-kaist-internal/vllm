...
echo "batch_size,context_len,num_query_heads,num_kv_heads,head_size,latency" > v1_output.csv
...
for config in "${configs[@]}"; do
    ...
    for batch_size in {1..256}; do
        ...
        # for context_len in 256 512 768 1024 1280 1536 1792 2048; do
            ...
            output=$(python benchmark_paged_attention.py \
                --version "v1" \
            ...
            echo "${batch_size},${context_len},${array[0]},${array[1]},${array[2]},${latency}" >> v1_output.csv
        done
    done
done

echo "batch_size,context_len,num_query_heads,num_kv_heads,head_size,latency" > v2_output.csv
...
for config in "${configs[@]}"; do
    ...
    for batch_size in {1..256}; do
        ...
        for context_len in 1024; do
            ...
            # Print the output with configuration, batch size, and context length
            echo "${batch_size},${context_len},${array[0]},${array[1]},${array[2]},${latency}" >> v2_output.csv
        done
    done
done
