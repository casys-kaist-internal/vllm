...
datasets=("finance_b" "finance")
temperatures=(0)
budgets=(512 1024 2048)

# Paths
python_script="benchmark_serving.py"
output_csv="benchmark_results_finance_b.csv"

initialize_csv() {
    if [ ! -f "$output_csv" ]; then
        echo "dataset,request_rate,draft_size,chunk_prefill,colocate,target_attention,budget,p50_ttft,p99_ttft,p50_tpot,p99_tpot,p50_tpt,p99_tpt,throughput,latency,preempt_flag" > "$output_csv"
    else
...
run_benchmark() {
    ...
    echo "Running benchmark for $dataset,  request_rate: $request_rate, draft_size: $draft_size, chunk_prefill: $chunk_prefill, colocate: $colocate, target_attention: $target_attention, budget: $budget"
    ...
    echo "python "$python_script" --dataset "$dataset" --request-rate "$request_rate" --draft-size "$draft_size" $chunk_prefill_flag $colocate_flag $target_attention_flag --budget $budget" > last_run.sh

    local output=$(python "$python_script" --dataset "$dataset" --request-rate "$request_rate" --draft-size "$draft_size" $chunk_prefill_flag $colocate_flag $target_attention_flag  --budget $budget)
    
    extract_values "$output"
    echo "$dataset,$request_rate,$draft_size,$chunk_prefill,$colocate,$target_attention,$budget,$p50_ttft,$p99_ttft,$p50_tpot,$p99_tpot,$p50_tpt,$p99_tpt,$throughput,$latency,$preempt_flag" >> "$output_csv"
    ./slack "dataset: $dataset, request_rate: $request_rate, draft_size: $draft_size, chunk_prefill: $chunk_prefill, colocate: $colocate, attention: $target_attention, budget: $budget, throughput: $throughput, latency: $latency, preempted: $preempt_flag"
}

# Initialize
initialize_csv

total_runs=$(( ${#datasets[@]} * ${#temperatures[@]} * ${#request_rates[@]} * ${#draft_sizes[@]} * ${#chunk_prefills[@]} * ${#colocates[@]} * ${#target_attentions[@]} * ${#budgets[@]}))
current_run=0
                    ...
                    for request_rate in "${request_rates[@]}"; do
                        for colocate in "${colocates[@]}"; do
                            for target_attention in "${target_attentions[@]}"; do
                                ...
                                run_benchmark "$dataset" "$request_rate" "$draft_size" "$chunk_prefill" "$colocate" "$target_attention" "$budget"
                                ((current_run++))
                            done
                        done
                    done
