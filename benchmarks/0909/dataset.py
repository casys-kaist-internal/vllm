...
def plot_length_distribution(name, data):
    ...
    # Calculate mean and standard deviation using statistics module
    prompt_mean = statistics.mean(prompt_lengths)
    prompt_std = statistics.stdev(prompt_lengths)
    prompt_median = statistics.median(prompt_lengths)
    ax[0].text(0.05, 0.95, f'Mean: {prompt_mean:.2f}\n Std Dev: {prompt_std:.2f}\n Median: {prompt_median}', transform=ax[0].transAxes,
               fontsize=12, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))

    output_mean = statistics.mean(output_lengths)
    output_std = statistics.stdev(output_lengths)
    output_median = statistics.median(output_lengths)
    ax[1].text(0.05, 0.95, f'Mean: {output_mean:.2f}\n Std Dev: {output_std:.2f}\n Median: {output_median}', transform=ax[1].transAxes,
               fontsize=12, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))
