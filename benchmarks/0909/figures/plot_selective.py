...
csv_file_name = "ours_selective_validation_full_prefill.csv"
...
for temp in temperatures:
    ...
    ax_throughput.plot(
        temp_data['drop_threshold'], temp_data['token_throughput'],
...
ax_throughput.set_ylabel('Token Throughput (tokens/s)', fontsize=10)
...
for temp in temperatures:
    ...
    # Plot the thicker black line as the "edge"
    ax_latency.plot(
        temp_data['drop_threshold'], temp_data['token_latency'],
        marker=marker_map[temp], color='black', linestyle='-', linewidth=2, alpha=0.7
    )
...
plt.savefig('token_throughput_latency_vs_drop_threshold_request_rate_12.pdf', bbox_inches='tight', format='pdf')
plt.show()
