...
def train_predictor(llm: SpecDecodeLLM, requests: List[Tuple[str, int, int]],
...
def run(llm: SpecDecodeLLM, requests: List[Tuple[str, int, int]],
        high_request_rate: float, mid_request_rate: float, low_request_rate: float,
    ...
    while True:
        ...
        for output in step_outputs:
            ...
            if output.finished:
                ...
                num_request = llm.llm_engine.scheduler.get_num_running_seq_groups()
    ...
    return result, total_tokens, llm.llm_engine.scheduler.preempt_flag, token_latencies_over_time, token_throughput_over_time, requests_over_time  # Return the throughput data
...
def main(args: argparse.Namespace):
    ...
    if args.selective_validation:
        ...
        train_predictor(llm, requests, args.high_request_rate, args.mid_request_rate, args.low_request_rate, args.temperature)
    else:
        warmup(llm)
    ...
    # Run the benchmark
    start_time = time.perf_counter()
    result, total_tokens, preempt_flag, token_latencies_over_time, token_throughput_over_time, requests_over_time = run(
    ...
    file_name_prefix = f"{args.colocate}_{args.selective_validation}"
