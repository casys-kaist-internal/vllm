...
df = pd.read_csv('figures/figure_1.csv')
...
subplot_division_features = ['target_model', 'draft_model', 'temperature']
...
for dataset in df['dataset'].unique():
    ...
    plt.savefig(
        f'figures/plot/benchmark_results_{dataset}.png', bbox_inches='tight')
    plt.close(fig)
...
for metric in ['p50_ttft', 'p99_ttft', 'p50_tpot', 'p99_tpot', 'p50_tpt', 'p99_tpt']:
    ...
    for dataset in df['dataset'].unique():
        ...
        # plt.tight_layout(rect=[0, 0.03, 1, 1])
        plt.savefig(
            f'figures/plot/benchmark_results_{metric}_{dataset}.png', bbox_inches='tight')
