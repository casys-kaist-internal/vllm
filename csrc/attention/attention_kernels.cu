...
#pragma unroll
      for (int i = 0; i < NUM_ROWS_PER_THREAD; i++)
      {
        ...
        if (row_idx < HEAD_SIZE)
        {
          ...
          V_vec v_vec = *reinterpret_cast<const V_vec *>(v_ptr + offset);
          scalar_t *v_vec_ptr = reinterpret_cast<scalar_t *>(&v_vec);
          for (int query_idx = query_lens[seq_idx] - 1 - warp_idx_y; query_idx >= 0; query_idx -= NUM_WARPS_Y)
          {
            // int n_context_len = context_len - (query_lens[seq_idx] - query_idx - 1);
            int n_context_len = context_len + query_idx;
...
#pragma unroll
            ...
            accs[query_idx / NUM_WARPS_Y][i] += dot(logits_vec, v_vec);
          }
        }
      }
    }
    ...
    for (int query_idx = query_lens[seq_idx] - 1 - warp_idx_y; query_idx >= 0; query_idx -= NUM_WARPS_Y)
    {
      ...
      for (int i = 0; i < NUM_ROWS_PER_THREAD; i++)
      {
        float acc = accs[query_idx / NUM_WARPS_Y][i];
        // #pragma unroll
        for (int mask = NUM_V_VECS_PER_ROW / 2; mask >= 1; mask /= 2)
        {
          acc += __shfl_xor_sync(uint32_t(-1), acc, mask);
        }
        accs[query_idx / NUM_WARPS_Y][i] = acc;
      }
    }
    // NOTE(woosuk): A barrier is required because the shared memory space for logits
    // is reused for the output.
...
#pragma unroll
          for (int i = 0; i < NUM_ROWS_PER_THREAD; i++)
          {
            ...
            if (row_idx < HEAD_SIZE && lane % NUM_V_VECS_PER_ROW == 0)
            {
              dst[row_idx] = accs[query_idx / NUM_WARPS_Y][i];
            }
          }
        }
      }
...
#pragma unroll
          for (int i = 0; i < NUM_ROWS_PER_THREAD; i++)
          {
            ...
            if (row_idx < HEAD_SIZE && lane % NUM_V_VECS_PER_ROW == 0)
            {
              accs[query_idx / NUM_WARPS_Y][i] += src[row_idx];
            }
          }
        }
      }
...
#pragma unroll
        for (int i = 0; i < NUM_ROWS_PER_THREAD; i++)
        {
          ...
          if (row_idx < HEAD_SIZE && lane % NUM_V_VECS_PER_ROW == 0)
          {
            from_float(*(out_ptr + row_idx), accs[query_idx / NUM_WARPS_Y][i]);
          }
        }
      }
    }
