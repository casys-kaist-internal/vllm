...
#pragma unroll
      ...
      const int kv_head_stride)
  {
    ...
    // We should have seq_idx and query_idx. Seq_idx is for each request and query_idx is for speculation window size.
    // Each thread_block should process a unique seq_idx as before but query_idx should be processed within a single thread_block.

    __shared__ Q_vec q_vecs[QUERY_SIZE][THREAD_GROUP_SIZE][NUM_VECS_PER_THREAD]; // NOTE(sjchoi): add 3 dimension for query_idx
#pragma unroll
    for (int query_idx = 0; query_idx < QUERY_SIZE; query_idx++) // NOTE(sjchoi): we should load all queries. Need to think about dynamic query_size.
    {
      const scalar_t *q_ptr = q + seq_idx * q_stride * QUERY_SIZE + query_idx * q_stride + head_idx * HEAD_SIZE;
...
#pragma unroll
        ...
        for (int query_idx = 0; query_idx < QUERY_SIZE; query_idx++)
        {
          ...
          if (thread_group_offset == 0)
          {
            ...
            logits_index = query_idx * num_tokens + (token_idx - start_token_idx);
...
#pragma unroll
      ...
      {
        logits_index = query_idx * num_tokens + i;
        float val = __expf(logits[logits_index] - qk_max[query_idx]);
        logits[logits_index] = val;
        exp_sum += val;
      }
