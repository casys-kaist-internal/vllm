...
#pragma unroll
    ...
    // Broadcast to other threads within warp
    return __shfl_sync(uint32_t(-1), sum, 0);
  }
...
#pragma unroll
      ...
      const int kv_head_stride)
  {
    ...
    // [start_token_idx, end_token_idx) is the range of tokens to process.
    const int start_token_idx = start_block_idx * BLOCK_SIZE;
    const int end_token_idx = MIN(start_token_idx + num_blocks * BLOCK_SIZE, max_context_len);
    ...
    __shared__ Q_vec q_vecs[QUERY_SIZE][THREAD_GROUP_SIZE][NUM_VECS_PER_THREAD]; // ~4KB
#pragma unroll
...
#pragma unroll
      ...
      {
        const int vec_idx = thread_group_offset + i * THREAD_GROUP_SIZE;
        q_vecs[query_idx][thread_group_offset][i] = *reinterpret_cast<const Q_vec *>(q_ptr + vec_idx * VEC_SIZE);
      }
    }
    __syncthreads(); // TODO(naed90): possible speedup if this is replaced with a memory wall right before we use q_vecs
...
#pragma unroll
          ...
          {
            q_vec_regs[it][j] = q_vecs[query_idx][thread_group_offset][v_outer * V_TILE_SIZE + j];
          }
        }
#pragma unroll
...
#pragma unroll
    ...
    {
      int query_idx = warp_idx_y_qk + it * NUM_WARPS_Y_QK;
      exp_sum_arr[it] = block_sum_query_group<NUM_WARPS_X_QK>(&red_smem[query_idx][NUM_WARPS_X_QK], exp_sum_arr[it]);
    }
...
#pragma unroll
  ...
  template <
      ...
      int NUM_THREADS,
      int PARTITION_SIZE = 0> // Zero means no partitioning.
  __device__ void paged_attention_target_kernel_tensor_core(
      float *__restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
      float *__restrict__ max_logits,       // [num_seqs, num_heads, max_num_partitions]
      scalar_t *__restrict__ out,           // [num_seqs, num_heads, max_num_partitions, head_size]
      const scalar_t *__restrict__ q,       // [num_seqs, num_heads, head_size]
      ...
      const int kv_head_stride)
  {
    ...
    /* Tensor Core Hyperparams */
    constexpr int WMMA_M = 8;  // Divides QUERY_SIZE
    constexpr int WMMA_N = 32; // Divides HEAD_SIZE
    constexpr int WMMA_K = 16; // Divides Context Length (PARTITION_SIZE)
    /* ----------------------- */
    ...
    // (hj) As logits is x QUERY_LEN, we need to pad to BLOCK_SIZE
    // const int PAD_MAX_NUM_TOKENS = DIVIDE_ROUND_UP(max_num_tokens, BLOCK_SIZE) * BLOCK_SIZE;
    constexpr int PAD_MAX_NUM_TOKENS = PARTITION_SIZE + WMMA_K / 2;
...
#pragma unroll
    ...
    /* FOR LV TENSOR CORE */
    scalar_t *logits_scalar_t = reinterpret_cast<scalar_t *>(shared_mem);
    float *output_shared = reinterpret_cast<float *>(logits_scalar_t + PAD_MAX_NUM_TOKENS * QUERY_SIZE); // Memory planning :)
    /* ------------------ */
...
#pragma unroll
    ...
    // const int WMMA_M = 8;  // Divides QUERY_SIZE
    // const int WMMA_N = 32; // Divides HEAD_SIZE
    // const int WMMA_K = 16; // Divides Context Length (PARTITION_SIZE)
    ...
    constexpr int M = QUERY_SIZE;
    constexpr int N = HEAD_SIZE;
    constexpr int K = PARTITION_SIZE;

    constexpr int NUM_C_WARPS = HEAD_SIZE / WMMA_N; // 4 for hz 128

    constexpr int NUM_K_WARPS = NUM_WARPS / NUM_C_WARPS;

    const int warp_idx_x_tc = warp_idx % NUM_C_WARPS;
    const int warp_idx_y_tc = warp_idx / NUM_C_WARPS;

    // #pragma unroll
    //     for (int iter = thread_idx; iter < PAD_MAX_NUM_TOKENS * QUERY_SIZE; iter += NUM_THREADS)
    //     {
    //       const int i = iter % PAD_MAX_NUM_TOKENS;
    //       const int query_idx = iter / PAD_MAX_NUM_TOKENS;
    //       const bool zero = i >= context_lens[cum_query_len + query_idx] || query_idx >= query_len;
    //       scalar_t scalar_t_zero;
    //       from_float(scalar_t_zero, 0.f);
    //       scalar_t f;
    //       from_float(f, logits[logits_idx<PAD_MAX_NUM_TOKENS>(query_idx, i)]);
    //       logits_scalar_t[2 * query_idx * PAD_MAX_NUM_TOKENS + i] = zero ? scalar_t_zero : f;
    //     }
    //     __syncthreads();

    for (int query_idx = warp_idx; query_idx < QUERY_SIZE; query_idx += NUM_WARPS)
    {
      for (int i = lane; i < PAD_MAX_NUM_TOKENS; i += WARP_SIZE)
      {
        const bool zero = i >= context_lens[cum_query_len + query_idx] || query_idx >= query_len;
        scalar_t scalar_t_zero;
        from_float(scalar_t_zero, 0.f);
    ...
    // Odd-number queries moved
    for (int query_idx = 1; query_idx < QUERY_SIZE; query_idx += 2)
    {
      for (int token_idx = thread_idx; token_idx < PARTITION_SIZE; token_idx += NUM_THREADS)
      {
        logits_scalar_t[query_idx * PAD_MAX_NUM_TOKENS + token_idx] = logits_scalar_t[2 * query_idx * PAD_MAX_NUM_TOKENS + token_idx];
      }
    }
    __syncthreads();

    // Even-number queries moved
    for (int query_idx = 0; query_idx < QUERY_SIZE; query_idx += 2)
    {
      for (int token_idx = thread_idx; token_idx < PARTITION_SIZE; token_idx += NUM_THREADS)
      {
        logits_scalar_t[query_idx * PAD_MAX_NUM_TOKENS + token_idx] = logits_scalar_t[2 * query_idx * PAD_MAX_NUM_TOKENS + token_idx];
    ...
    for (int query_idx = 0; query_idx < QUERY_SIZE; query_idx++)
    {
      if (query_idx >= query_len)
        continue;
      scalar_t *out_ptr = out + (cum_query_len + query_idx) * num_heads * max_num_partitions * HEAD_SIZE + head_idx * max_num_partitions * HEAD_SIZE + partition_idx * HEAD_SIZE;
      for (int w = lane; w < WMMA_N; w += WARP_SIZE)
      {
        constexpr int kc = 0;
        const int idx = warp_idx_x_tc * WMMA_N + w;
        float res = output_shared[warp_idx_x_tc * QUERY_SIZE * WMMA_N_PAD + query_idx * WMMA_N_PAD + w];
        from_float(*(out_ptr + idx), res);
      }
    }
  }
      ...
      const int kv_head_stride)
  {
    paged_attention_target_kernel<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>(
        /* exp_sums */ nullptr, /* max_logits */ nullptr,
        out, q, k_cache, v_cache, head_mapping, scale, block_tables, context_lens, query_lens,
        max_num_blocks_per_seq, alibi_slopes, q_stride, kv_block_stride, kv_head_stride);
  }
  ...
  template <
      ...
      int PARTITION_SIZE>
  __global__ void
  paged_attention_v2_target_kernel(
      float *__restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
      float *__restrict__ max_logits,       // [num_seqs, num_heads, max_num_partitions]
      scalar_t *__restrict__ tmp_out,       // [num_seqs, num_heads, max_num_partitions, head_size]
      const scalar_t *__restrict__ q,       // [num_seqs, num_heads, head_size]
      ...
      const int kv_head_stride)
  {
    paged_attention_target_kernel<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE>(
        exp_sums, max_logits, tmp_out, q, k_cache, v_cache, head_mapping, scale,
        block_tables, context_lens, query_lens, max_num_blocks_per_seq, alibi_slopes,
        q_stride, kv_block_stride, kv_head_stride);
  }
  ...
  template <
      ...
      int PARTITION_SIZE>
  __global__ void
  paged_attention_v2_target_kernel_tensor_core(
      float *__restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
      float *__restrict__ max_logits,       // [num_seqs, num_heads, max_num_partitions]
      scalar_t *__restrict__ tmp_out,       // [num_seqs, num_heads, max_num_partitions, head_size]
      const scalar_t *__restrict__ q,       // [num_seqs, num_heads, head_size]
      ...
      const int kv_head_stride)
  {
    paged_attention_target_kernel_tensor_core<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE>(
        exp_sums, max_logits, tmp_out, q, k_cache, v_cache, head_mapping, scale,
        block_tables, context_lens, query_lens, max_num_blocks_per_seq, alibi_slopes,
        q_stride, kv_block_stride, kv_head_stride);
  }
      ...
      const int max_num_partitions)
  {
    ...
    if (num_partitions == 1)
    {
      for (int query_idx = 0; query_idx < query_len; query_idx++)
      {
        ...
        const scalar_t *tmp_out_ptr = tmp_out + (cum_query_len + query_idx) * num_heads * max_num_partitions * HEAD_SIZE + head_idx * max_num_partitions * HEAD_SIZE;
        for (int i = threadIdx.x; i < HEAD_SIZE; i += blockDim.x)
        {
          out_ptr[i] = tmp_out_ptr[i];
        }
...
#pragma unroll
      for (int i = threadIdx.x; i < HEAD_SIZE; i += NUM_THREADS)
      {
        ...
        for (int j = 0; j < num_partitions; ++j)
        {
          acc += to_float(tmp_out_ptr[j * HEAD_SIZE + i]) * shared_exp_sums[query_idx * num_partitions + j] * inv_global_exp_sum;
        }
        from_float(out_ptr[i], acc);
      }
    }
...
#define LAUNCH_PAGED_ATTENTION_V1_TARGET(HEAD_SIZE)                                  \
  cudaFuncSetAttribute(                                                              \
      vllm::paged_attention_v1_target_kernel<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>, \
      cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);                 \
  vllm::paged_attention_v1_target_kernel<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>      \
      <<<grid, block, shared_mem_size, stream>>>(                                    \
          out_ptr,                                                                   \
...
#define LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(HEAD_SIZE)                                             \
  vllm::paged_attention_v2_target_kernel_tensor_core<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE> \
      <<<grid, block, shared_mem_size, stream>>>(                                                           \
          exp_sums_ptr,                                                                                     \
          max_logits_ptr,                                                                                   \
          tmp_out_ptr,                                                                                      \
...
template <
    ...
    int NUM_THREADS = 128,
    int PARTITION_SIZE = 512>
void paged_attention_v2_target_launcher_tensor_core(
    torch::Tensor &out,
    torch::Tensor &exp_sums,
    torch::Tensor &max_logits,
    torch::Tensor &tmp_out,
    ...
    const c10::optional<torch::Tensor> &alibi_slopes)
{
  ...
  int pad_max_num_tokens = PARTITION_SIZE + 8;

  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int max_num_partitions = DIVIDE_ROUND_UP(max_context_len, PARTITION_SIZE);
  int logits_base_size = QUERY_SIZE * pad_max_num_tokens * sizeof(float);

  int logits_reduced_size = QUERY_SIZE * pad_max_num_tokens * sizeof(T);
  int outputs_shared_size = NUM_WARPS * QUERY_SIZE * 32 * sizeof(float);
  int values_shared_size = 0; // 16 * head_size * sizeof(T);

  // [<-----        logits_size     ------>]
  // [logits_reduced_size][outputs_shared_size][v_shared_cache]

  int phase_a = logits_base_size;
  int phase_b = logits_reduced_size + outputs_shared_size + values_shared_size;

  // int outputs_size = QUERY_SIZE * NUM_WARPS * head_size * sizeof(float);

  // For paged attention v2 kernel.
  dim3 grid(num_heads, num_seqs, max_num_partitions);
  int shared_mem_size = std::max(phase_a, phase_b);

  // print shared_mem_size and its elements
  ...
  // to support any head size which is a multiple of 16.
  // case 64:
  //   LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(64);
  //   break;
  // case 80:
  //   LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(80);
  //   break;
  // case 96:
  //   LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(96);
  //   break;
  // case 112:
  //   LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(112);
  //   break;
  case 128:
    LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(128);
    break;
  // case 256:
  //   LAUNCH_PAGED_ATTENTION_V2_TARGET_TENSOR_CORE(256);
  //   break;
  default:
    TORCH_CHECK(false, "Unsupported head size: ", head_size);
    break;
...
#define CALL_V2_TARGET_LAUNCHER_TENSOR_CORE_BLOCK_SIZE(T)       \
  switch (block_size)                                           \
  {                                                             \
  case 32:                                                      \
    CALL_V2_TARGET_LAUNCHER_TENSOR_CORE(T, 32);                 \
    break;                                                      \
  default:                                                      \
    TORCH_CHECK(false, "Unsupported block size: ", block_size); \
    break;                                                      \
...
void paged_attention_v2_target_tensor_core(
    torch::Tensor &out,          // [num_seqs, num_heads, head_size]
    torch::Tensor &exp_sums,     // [num_seqs, num_heads, max_num_partitions]
    torch::Tensor &max_logits,   // [num_seqs, num_heads, max_num_partitions]
    torch::Tensor &tmp_out,      // [num_seqs, num_heads, max_num_partitions, head_size]
    ...
    const c10::optional<torch::Tensor> &alibi_slopes)
{
  if (query.dtype() == at::ScalarType::Half)
  {
    CALL_V2_TARGET_LAUNCHER_TENSOR_CORE_BLOCK_SIZE(uint16_t);
  }
  // else if (query.dtype() == at::ScalarType::BFloat16)
  // {
  //   CALL_V2_TARGET_LAUNCHER_TENSOR_CORE_BLOCK_SIZE(__nv_bfloat16);
  // }
  else if (query.dtype() == at::ScalarType::Float)
  {
    CALL_V2_TARGET_LAUNCHER_TENSOR_CORE_BLOCK_SIZE(float);
  }
  else
  {
    TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
