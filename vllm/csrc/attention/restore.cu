...
//     constexpr int NUM_BLOCKS_PER_WARP = WARP_SIZE * NUM_TOKENS_PER_THREAD_GROUP / BLOCK_SIZE;
...
//     constexpr int NUM_ELEMS_PER_THREAD = HEAD_SIZE / THREAD_GROUP_SIZE;
//     constexpr int NUM_VECS_PER_THREAD = NUM_ELEMS_PER_THREAD / VEC_SIZE;
//     constexpr int NUM_VECS_PER_THREAD_TILE = 4; // Process 4 vectors each (out of 8)
...
//     __shared__ K_vec k_vecs[NUM_BLOCKS_PER_WARP][BLOCK_SIZE][THREAD_GROUP_SIZE][NUM_VECS_PER_THREAD];
...
//     // Collaboratively zero-out logits
...
//     constexpr int x = 16 / sizeof(scalar_t);
//     // float qk_max = -FLT_MAX;
//     __shared__ float qk_max[QUERIES_PER_WARP_GROUP_QK][NUM_THREADS];
...
//     {
//       const int block_idx = warp_block_idx_start + (lane * NUM_TOKENS_PER_THREAD_GROUP) / BLOCK_SIZE;
...
//       // (e.g., kv_block_stride).
//       const int64_t physical_block_number = static_cast<int64_t>(block_table[block_idx]);
//       float logits_reg[QUERIES_PER_WARP_GROUP_QK][NUM_TOKENS_PER_THREAD_GROUP] = {0.f};

//       for (int jm = 0; jm < NUM_VECS_PER_THREAD / NUM_VECS_PER_THREAD_TILE; jm++)
//       {
//         Q_vec q_vec_regs[QUERIES_PER_WARP_GROUP_QK][NUM_VECS_PER_THREAD_TILE] = {0.f};
//         K_vec k_vec_regs[NUM_TOKENS_PER_THREAD_GROUP][NUM_VECS_PER_THREAD_TILE] = {0.f};

// #pragma unroll
//         for (int it = 0; it < QUERIES_PER_WARP_GROUP_QK; it++)
//         {
// #pragma unroll
//           for (int jt = 0; jt < NUM_VECS_PER_THREAD_TILE; jt++)
//           {
//             int query_idx = warp_idx_y_qk + it * NUM_WARPS_Y_QK;
//             q_vec_regs[it][jt] = q_vecs[query_idx][thread_group_offset][jm * NUM_VECS_PER_THREAD_TILE + jt];
//           }
//         }
//         // Load q_vec_regs
// #pragma unroll
...
//           for (int jt = 0; jt < NUM_VECS_PER_THREAD_TILE; jt++)
//           {
//             int j = jm * NUM_VECS_PER_THREAD_TILE + jt;
//             const scalar_t *k_ptr = k_cache + physical_block_number * kv_block_stride + kv_head_idx * kv_head_stride + physical_block_offset * x;
//             const int vec_idx = thread_group_offset + j * THREAD_GROUP_SIZE;
//             const int offset1 = (vec_idx * VEC_SIZE) / x;
//             const int offset2 = (vec_idx * VEC_SIZE) % x;
//             k_vec_regs[i][jt] = *reinterpret_cast<const K_vec *>(k_ptr + offset1 * BLOCK_SIZE * x + offset2);
//             // k_vec_regs[i][jt] = k_vecs[block_idx - warp_block_idx_start][physical_block_offset][thread_group_offset][j];
//           }
// #pragma unroll
//           for (int it = 0; it < QUERIES_PER_WARP_GROUP_QK; it++)
//           {
...
//             {
//               int j = jm * NUM_VECS_PER_THREAD_TILE + jt;
//               A_vec q_vec = *reinterpret_cast<A_vec *>(&q_vec_regs[it][jt]);
//               A_vec k_vec = *reinterpret_cast<A_vec *>(&k_vec_regs[i][jt]);
//               qk += dot(q_vec, k_vec);
//             }
//             qk *= scale;
//             qk += (alibi_slope != 0) ? alibi_slope * (token_idx - n_context_len + 1) : 0;
//             const bool mask = token_idx >= n_context_len;
//             // logits_reg[it][i] += mask ? 0.f : qk;
//             logits[logits_idx(PAD_MAX_NUM_TOKENS, query_idx, tok_offset)] += mask ? 0.f : qk; // TODO TODO
//             qk_max_reg[it] = mask ? qk_max_reg[it] : fmaxf(qk_max_reg[it], qk);
//           } // QUERIES_PER_WARP_GROUP_QK end
//         }   // NUM_TOKENS_PER_THREAD_GROUP end
//       }     // jm end
//       for (int it = 0; it < QUERIES_PER_WARP_GROUP_QK; it++)
//       {
//         int query_idx = warp_idx_y_qk + it * NUM_WARPS_Y_QK;
...
//           // logits[logits_index] = logits_reg[it][i];
//         }
//       }
//     } // block iterator end
...
//     // for (int query_idx = 0; query_idx < QUERY_SIZE; query_idx++)
//     // {
//     //   qk_max[it][thread_idx] = qk_max[it][thread_idx];
//     // }
...
//     float exp_sum_arr[QUERIES_PER_WARP_GROUP_QK];
...
//       for (int i = thread_idx - (warp_idx_y_qk * NUM_WARPS_X_QK * WARP_SIZE); i < num_tokens; i += NUM_THREADS / NUM_WARPS_Y_QK)
//       {
...
//         exp_sum += val;
//       }
//       exp_sum = block_sum_query_group<NUM_WARPS_X_QK>(&red_smem[query_idx][NUM_WARPS_X_QK], exp_sum);

//       exp_sum_arr[it] = exp_sum;
