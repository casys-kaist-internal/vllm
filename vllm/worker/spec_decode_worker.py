...
class SpecDecodeWorker:
    ...
    def execute_multi_step_model(
        ...
        num_steps = seq_group_metadata_list[0].draft_size

        for _ in range(num_steps):
            output = self.model_runner.execute_model(seq_group_metadata_list,
                                                     self.gpu_cache)
            self._process_model_outputs(output, seq_group_metadata_list)
