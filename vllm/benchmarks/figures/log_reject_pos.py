...
def load_gsm8k():
    return load_requests_from_file(f'{dataset_download_dir}/gsm8k_7473.json')

def load_humaneval():
    return load_requests_from_file(f'{dataset_download_dir}/humaneval_164.json')

def load_alpaca():
    return load_requests_from_file(f'{dataset_download_dir}/alpaca_9669.json')

def load_mt_bench():
    return load_requests_from_file(f'{dataset_download_dir}/mt-bench_79.json')

def load_sharegpt():
    return load_requests_from_file(f'{dataset_download_dir}/sharegpt_10690.json')

def load_apps():
    return load_requests_from_file(f'{dataset_download_dir}/apps_4849.json')
...
def main(args: argparse.Namespace):
    ...
    if args.dataset == "gsm8k":
        ...
        requests = load_humaneval()
    elif args.dataset == "alpaca":
        requests = load_alpaca()
    elif args.dataset == "mt-bench":
        requests = load_mt_bench()
    elif args.dataset == "sharegpt":
        requests = load_sharegpt()
    elif args.dataset == "apps":
        requests = load_apps()
    elif args.dataset == "all":
        requests = load_all_datasets()
    else:
        raise ValueError(f"Unknown dataset: {args.dataset}")
    

    # sort request by request_id 
    ...
    for request in requests:
        ...
        batch.append((request_id, prompt, None, sampling_params))
        
        # Check if batch is full
        if len(batch) == batch_size:
            # Process the full batch
            for req_id, prmpt, prmpt_ids, s_params in batch:
                llm._add_request(
                    request_id=req_id,
                    prompt=prmpt,
    ...
    # sort result by request_id
    result = sorted(result, key=lambda x: x.request_id)

    total_result = []
    for output in result:
        request_id = output.request_id
        reject_pos = output.outputs[0].reject_pos
        accept_probs = output.outputs[0].accept_probs
