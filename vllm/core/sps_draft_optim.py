...
class BetaEMADraftSizeOptimizer(DraftSizeOptimizer):
    ...
    def _get_seq_features(self, seq: Sequence) -> List[float]:
        ...
        if self.retrain and self.retrain_index >= self.retrain_period:
            ...
            self._train_predictor(seq.temperature)
            nvtx.range_pop()
    ...
    def _train_predictor(self, temperature):
        ...
        if self.sps_config.use_lookup_table:
            self.lookup_table[temperature].fill(0) 
            beta_indices = np.digitize(X[:, 0], self.linspace) - 1
            draft_indices = np.digitize(X[:, 1], self.linspace) - 1

            if self.sps_config.predictor_agg_type == "mean":
                np.add.at(self.lookup_table[temperature], (beta_indices, draft_indices), y)
                counts = np.zeros_like(self.lookup_table[temperature])
                np.add.at(counts, (beta_indices, draft_indices), 1)
                self.lookup_table[temperature] = np.divide(self.lookup_table[temperature], counts, where=counts != 0)
            elif self.sps_config.predictor_agg_type == "median":
                for i in range(self.num_bins):
                    for j in range(self.num_bins):
                        mask = (beta_indices == i) & (draft_indices == j)
                        if mask.any():
                            self.lookup_table[temperature][i, j] = np.median(y[mask])
        ...
        if PLOT_HEATMAP:
            if self.sps_config.use_lookup_table:
                y_predict = self.lookup_table[temperature]
            else:
